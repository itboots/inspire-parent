...
import com.utaka.inspire.util.Casting;
...
public final class JpqlTokens {
    private static AccessibleObjectPredicate SELECT_COUNT_PREDICATE = new AccessibleObjectPredicate(SelectCountToken.class);
    ...
    static {
        collectors.add(new Collector() {
            ...
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(SelectCountPart.class)) {
        ...
        collectors.add(new Collector() {
            @Override
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(SelectPart.class)) {
        ...
        collectors.add(new Collector() {
            ...
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(FilterPart.class)) {
                    ...
                    if (val == null)//如果值为空字符串,则作为常量过滤条件处理。
                        return Optional.absent();
        ...
        collectors.add(new Collector() {
            @Override
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(GroupByPart.class)) {
        ...
        collectors.add(new Collector() {
            @Override
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(HavingPart.class)) {
        ...
        collectors.add(new Collector() {
            @Override
            public <T extends JpqlToken> Optional<T> collect(Field f, Object object) {
                if (f.isAnnotationPresent(OrderByPart.class)) {
                    Object val = value(object, f);
                    if (val == null) {
                        return Optional.absent();
                    }
